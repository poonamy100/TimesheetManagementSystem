...
namespace TimesheetManagementSystemUI.Controllers
{
    public class LocationsController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                using (var response = await httpClient.GetAsync("http://localhost:5283/api/Locations/Get"))
                {
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("http://localhost:5283/api/Locations/Details/" + id))
        ...
        public IActionResult Create(Location location)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var postTask = httpClient.PostAsJsonAsync<Location>("http://localhost:5283/api/Locations/Create/", location);
                postTask.Wait();
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            HttpResponseMessage responseMessage = await client.GetAsync("http://localhost:5283/api/Locations/Details/" + id);
        ...
        public async Task<ActionResult> Edit(int id, Location loc)
        {
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage responseMessage = await client.PutAsJsonAsync("http://localhost:5283/api/Locations/Edit/" + id, loc);
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("Index");
                }
        ...
        public async Task<ActionResult> Delete(int id, Location Loc)
        {
            using (var client = new HttpClient())
            {
                HttpResponseMessage responseMessage = await client.DeleteAsync("http://localhost:5283/api/Locations/Delete/" + id);
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("Index");
                }
